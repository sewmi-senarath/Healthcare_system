...
const commonEmployeeValidation = [
  ...
  body('position').trim().isLength({ min: 2, max: 50 })
];

router.post('/doctor/register', commonEmployeeValidation, (req, res) => {
  const error = checkValidationErrors(req, res);
  if (error) return error;
  const employeeController = container.get('employeeController');
  employeeController.registerEmployee(req, res, 'doctor');
});

router.post('/nurse/register', commonEmployeeValidation, (req, res) => {
  const error = checkValidationErrors(req, res);
  if (error) return error;
  const employeeController = container.get('employeeController');
  employeeController.registerEmployee(req, res, 'nurse');
});

router.post('/pharmacist/register', commonEmployeeValidation, (req, res) => {
  const error = checkValidationErrors(req, res);
  if (error) return error;
  const employeeController = container.get('employeeController');
  employeeController.registerEmployee(req, res, 'pharmacist');
});

router.post('/hospital-staff/register', commonEmployeeValidation, (req, res) => {
  const error = checkValidationErrors(req, res);
  if (error) return error;
  const employeeController = container.get('employeeController');
  employeeController.registerEmployee(req, res, 'hospitalStaff');
});

router.post('/healthcare-manager/register', commonEmployeeValidation, (req, res) => {
  const error = checkValidationErrors(req, res);
  if (error) return error;
  const employeeController = container.get('employeeController');
  employeeController.registerEmployee(req, res, 'healthCareManager');
});
...
router.get('/profile', authenticateToken, (req, res) => {
  const employeeController = container.get('employeeController');
  employeeController.getEmployeeProfile(req, res);
});
...
router.get('/:employeeType', authenticateToken, (req, res) => {
  const employeeController = container.get('employeeController');
  employeeController.getAllEmployees(req, res);
});

router.get('/:employeeType/:empID', authenticateToken, (req, res) => {
  const employeeController = container.get('employeeController');
  employeeController.getEmployeeByEmpID(req, res);
});

router.put('/:employeeType/:empID/status', authenticateToken, (req, res) => {
  const employeeController = container.get('employeeController');
  employeeController.updateEmployeeStatus(req, res);
});
...
router.get('/dashboard/recent-activity', authenticateToken, (req, res) => {
  const employeeController = container.get('employeeController');
  employeeController.getRecentActivity(req, res);
});
