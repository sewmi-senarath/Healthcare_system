...
export class PatientController {
  ...
  async getPatientProfile(req, res) {
    try {
      const patientId = req.user.userId || req.params.patientId;
  ...
  async updatePatientProfile(req, res) {
    try {
      ...
      const patient = await this.patientService.updatePatientProfile(patientId, req.body);
  ...
  async getRecentActivity(req, res) {
    try {
      const patientId = req.user._id;
      const activities = await this.patientService.getRecentActivity(patientId);
  ...
  async viewMedicalRecords(req, res) {
    try {
      const patientId = req.user.userId;
      const records = await this.patientService.viewMedicalRecords(patientId);
      res.json({ success: true, medicalRecords: records });
    } catch (error) {
      console.error('View medical records error:', error);
      res.status(500).json({ success: false, message: 'Internal server error' });
    }
  ...
  async updateMedicalHistory(req, res) {
    try {
      ...
      const result = await this.patientService.updateMedicalHistory(patientId, medicalHistory);
      res.json(result);
    } catch (error) {
      console.error('Update medical history error:', error);
      res.status(500).json({ success: false, message: 'Internal server error' });
  ...
  async updateAllergies(req, res) {
    try {
      ...
      const result = await this.patientService.updateAllergies(patientId, allergies);
      res.json(result);
    } catch (error) {
      console.error('Update allergies error:', error);
      res.status(500).json({ success: false, message: 'Internal server error' });
  ...
  async getPatientAppointments(req, res) {
    try {
      const patientId = req.user._id;
      const appointments = await this.patientService.getPatientAppointments(patientId);
  ...
  async getPatientNotifications(req, res) {
    try {
      const patientId = req.user._id;
      const notifications = await this.patientService.getPatientNotifications(patientId);
  ...
  async getPatientMedicalRecords(req, res) {
    try {
      const patientId = req.user._id;
      const records = await this.patientService.getPatientMedicalRecords(patientId);
      res.json({ success: true, medicalRecords: records });
    } catch (error) {
  ...
  async searchPatientById(req, res) {
    try {
      const { patientId } = req.params;
      const patient = await this.patientService.searchPatientById(patientId);
      res.json({ success: true, patient });
    } catch (error) {
