...
export class EmployeeController {
  constructor(employeeServiceFactory) {
    this.employeeServiceFactory = employeeServiceFactory;
  }

  async registerEmployee(req, res, employeeType) {
    try {
      const type = employeeType || req.body.userType || req.params.employeeType;
      const employeeService = this.employeeServiceFactory.getService(type);
  ...
  async loginEmployee(req, res) {
    try {
      ...
      const employeeService = this.employeeServiceFactory.getService(userType);
  ...
  async updateEmployeeProfile(req, res) {
    try {
      ...
      const userType = req.user.userType;
      const employeeService = this.employeeServiceFactory.getService(userType);
      const employee = await employeeService.updateProfile(employeeId, req.body);
  ...
  async getAllEmployees(req, res) {
    try {
      ...
      const employeeService = this.employeeServiceFactory.getService(employeeType);
  ...
  async getEmployeeByEmpID(req, res) {
    try {
      const { employeeType, empID } = req.params;
      const employeeService = this.employeeServiceFactory.getService(employeeType);
      const employee = await employeeService.getEmployeeByEmpID(empID);
  ...
  async updateEmployeeStatus(req, res) {
    try {
      ...
      const employeeService = this.employeeServiceFactory.getService(employeeType);
      const result = await employeeService.updateEmployeeStatus(empID, status, reason);
      
      res.json({ success: true, message: 'Status updated successfully', employee: result });
    } catch (error) {
      console.error('Update employee status error:', error);
      res.status(400).json({ success: false, message: error.message });
    }
  ...
  async getRecentActivity(req, res) {
    try {
      ...
      const userType = req.query.userType || req.user.userType;
      const employeeService = this.employeeServiceFactory.getService(userType);
      const activities = await employeeService.getRecentActivity(employeeId);
